...
def network(params,train_input,train_predict):
    ...
    encoder = ConvLSTM2D(filters=params['filter'], kernel_size=(3, 3), padding='same',return_sequences=True,return_state=True, kernel_initializer='he_uniform',activity_regularizer=regularizers.l1(params["regularizer"]),name='encoder')
    BN1=BatchNormalization(name='BN1')
    BN2=BatchNormalization(name='BN2')
    encoder_outputs, _, state_c = encoder(encoder_inputs)
    encoder_outputs=BN1(encoder_outputs)
    state_c_use=BN2(state_c)
    state_h_use=Lambda(lambda x: x[:, -1, :, :, :])(encoder_outputs)
    # print(encoder_outputs,encoder_inputs)
    # We discard `encoder_outputs` and only keep the states.
    # encoder_states = [state_h, state_c]  #copy的最近一个时刻的状态值

    ##############################################    Dictionary Meomory 模块    ##############################################
    # memory 层定义:寻址过程
    ...
    past_decoder_outputs = rec_convlstm_decoder(encoder_outputs)
    ...
    '''全局共用层'''
    '''长度>1专用'''
    pre_convlstm_decoder = ConvLSTM2D(filters=params['filter'], kernel_size=(3, 3), padding='same', return_state=True,
                                  return_sequences=False, kernel_initializer='he_uniform', name='pre_convlstm_decoder')
    ...
    if train_predict.shape[1] == 1:
        ...
        encoder_outputs_T = encoder_outputs_for_mul(encoder_outputs)   # 已经改成经过memory的encoder
        ...
        COMPOSITE_ED.compile(loss=[params['loss1'], params['loss2']], loss_weights=[params['rcstr_weight'], params['predict_weight']], optimizer=rmsprop)
    ...
    ##########    长度>1的变窗预测（特殊）    ##########
        ...
        for i in range(max_decoder_seq_length):
            ...
            # 两者做 batch_dot
            context_vector = context_vector_mul(encoder_outputs_T)  # shape=(?, 40, 51, 64)
            # shape=(?, 1, 40, 51, 64)
            '''<END> 求出context_vector'''
            '''external memory in decoder'''
            '''将上一步的输出concatenate进context_vector-conditioning'''
            ...
            output, state_in_h, state_in_c = pre_convlstm_decoder([addressed_concat_inputs, state_in_h, state_in_c])
            output = BN4(output)
            output = decoder_dense_pre(output)  # 把通道数降为1
            output = output_reshape(output)
            # 将此时的output复制给fu_decoder_inputs
        ...
        COMPOSITE_ED = Model(inputs=[encoder_inputs, fu_decoder_inputs],outputs=[past_decoder_outputs, future_decoder_outputs])
        # COMPOSITE_ED.summary()
        rmsprop = optimizers.RMSprop(lr=params['lr'], rho=0.9, epsilon=None, decay=0.0)
