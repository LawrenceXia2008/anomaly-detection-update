...
def NN_predict(model,dataX,dataY):
    ...
    initial_s = np.zeros((dataX.shape[0], 1, dataX.shape[2], dataX.shape[3], 1))  # (smaple,1,20,51,1)
...
def network(params,train_input,train_predict):
    ...
    ##############################################    ENCODER    #############################################
    # try:
    encoder_inputs = Input(shape=(train_input.shape[1], train_input.shape[2],train_input.shape[3], 1),name='encoder_input')
    ...
    decoder_recon = ConvLSTM2D(filters=params['filter'], kernel_size=(3, 3), padding='same', return_sequences=True, kernel_initializer='he_uniform', name='decode_con')
    BN3 = BatchNormalization(name="BN3")
    decoder_outputs = decoder_recon(memoried_encoder_outputs)
    ...
    decoder_outputs=BN3(decoder_outputs)
    ...
    if train_predict.shape[1]==1:
        '''长度=1的预测时'''
        decoder_lstm_pre = ConvLSTM2D(filters=params['filter'], kernel_size=(3, 3), padding='same', kernel_initializer='he_uniform', name='decode_pre')
        decoder_dense_pre = Conv2D(filters=1, kernel_size=(1, 1), activation=params['activation'], padding='same',data_format='channels_last', kernel_initializer='he_uniform')
        decoder_outputs_pre=decoder_lstm_pre(encoder_outputs)
        decoder_outputs_pre=decoder_dense_pre(decoder_outputs_pre)
        decoder_outputs_pre=Reshape((train_predict.shape[1], train_predict.shape[2], train_predict.shape[3], 1))(decoder_outputs_pre)

        ##############################################    Model saving    ##############################################
        COMPOSITE_ED = Model(inputs=[encoder_inputs, past_decoder_inputs],
                             outputs=[decoder_outputs, decoder_outputs_pre])
        ...
        COMPOSITE_ED.compile(loss=[params['loss1'], params['loss2']], loss_weights=[params['rcstr_weight'], params['predict_weight']],
                             optimizer=rmsprop)  # 现在就表示相加
    else:
        '''有1<T<input_length长度的预测时'''
        ...
        decoder_pre_outputs=decoder_pre(memoried_encoder_outputs)
        ...
        COMPOSITE_ED = Model(inputs=encoder_inputs,
                             outputs=[decoder_outputs, decoder_pre_outputs])
        # COMPOSITE_ED.summary()
        rmsprop = optimizers.RMSprop(lr=params['lr'], rho=0.9, epsilon=None, decay=0.0)
        COMPOSITE_ED.compile(loss=[params['loss1'], params['loss2']], loss_weights=[params['rcstr_weight'], params['predict_weight']],
                             optimizer=rmsprop)
...
def model_operation(COMPOSITE_ED):
    ...
    #permutation=np.load("./permutationlist.npy")
    ...
    initial = np.zeros((train_input.shape[0], 1, train_input.shape[2],train_input.shape[3], 1))  # (smaple,1,20,51,1)
